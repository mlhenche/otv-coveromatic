...
- Files: `v3/src/code.ts` — `applyProviderLogo` (~L198), `isProviderLogoComponent` (~L246), `findProviderLogoAncestor` (~L271)
...
- Files: `v3/src/ui/components/HtmlPasteTab.tsx` — `handleApplyAll` (~L343), `v3/src/code.ts` — `apply-multiple-covers-url` handler (~L1002)
...
- Files: `v3/src/code.ts` L210, L1054, L1058, L1060, L1065, L1073
...
- Files: `v3/src/ui/components/HtmlPasteTab.tsx`, `v3/src/ui/components/CoverGrid.tsx`
...
- Files: `v3/src/ui/components/CoverGrid.tsx` (L43, L44, L58, L75, L169, L228), `v3/src/ui/components/HtmlPasteTab.tsx` (L36)
...
- Files: `v3/src/ui/components/CoverGrid.tsx` (L475, L491)
...
- Issue: `code.ts` handles `cache-catalog` (L819) and `get-cached-catalog` (L834) messages to persist catalog data, but `useSupabaseCatalog.ts` never sends these messages. Only `@tanstack/react-query` with `staleTime: 4h` is used. Closing and reopening the plugin always triggers a full Supabase fetch.
- Files: `v3/src/code.ts` (L819–856), `v3/src/ui/hooks/useSupabaseCatalog.ts`
- Impact: Dead code in `code.ts`; the intended offline/resilience behavior is inactive.
- Fix: Either wire up the cache in `useSupabaseCatalog.ts` (check `figma.clientStorage` before fetching, populate it after) or remove the dead handlers.
...
- Recommendation: Remove from `CLAUDE.md`. Reference `v3/scripts/.env.example` or `figma.clientStorage` instead.
...
- Files: `.claude/CLAUDE.md`
- Current mitigation: The key is only used by CLI scripts (`v3/scripts/`); it is not compiled into the plugin.
- Recommendation: Remove from `CLAUDE.md`. Store only in a local `.env` file excluded by `.gitignore`. `v3/scripts/.env.example` exists for this purpose.
...
- Risk: The Supabase anon key is hardcoded in `useSupabaseCatalog.ts` L4 and compiled into the distributed `v3/plugin/ui.html`.
- Files: `v3/src/ui/hooks/useSupabaseCatalog.ts` (L4), `v3/plugin/ui.html`
...
- Recommendation: Document the active RLS policies in `v3/supabase/schema.sql` to confirm they enforce read-only access for the anon role.
...
- Files: `v3/src/ui/components/HtmlPasteTab.tsx` (L351–364)
...
- Files: `v3/src/code.ts` (L1018–1077)
...
- Files: `v3/src/code.ts` (L372–397, L288–303)
...
- Files: `v3/src/code.ts` — `isCoverNode` (L34), `isTitleTreatmentNode` (L38), `findTextNode` (L45), `findInstanceNode` (L66), `METADATA_NODE_NAMES` (L605)
...
- Files: `v3/src/code.ts` — `getChannelRowOffset` (L993)
...
- Files: `v3/src/ui/App.tsx` (L25–27), `v3/src/ui/components/HtmlPasteTab.tsx`
...
- Files: `v3/src/ui/components/HtmlPasteTab.tsx` — `CARD_PARSERS` (L222), `parseCardChannel` (L117), `parseCardEmission` (L93), `parseCardGeneric` (L167)
...
- Files: `v3/src/code.ts` — `sendSelection` (L677), stale checks at L694, L700, L705, L716
...
- Files: `v3/src/ui/components/HtmlPasteTab.tsx` — `parseCardEmission` (L93)
...
- Files: `v3/src/ui/App.tsx`, `v3/src/ui/main.tsx`
...
- Files: `v3/src/ui/components/HtmlPasteTab.tsx` — `handleParse` (L295)
...
- Files: All of `v3/src/` — zero test files exist.
