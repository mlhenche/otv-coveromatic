...
function findBestVariantMatch(providerValue, variantOptions) {
    ...
    if (mapped && variantOptions.includes(mapped))
        return mapped;
    // 1. Exact match
    if (variantOptions.includes(providerValue))
...
function isProviderLogoComponent(node) {
    if (node.type !== 'INSTANCE')
        return false;
    const name = node.name.trim().toLowerCase();
    if (name === 'providerlogosquare' || name === 'providerlogorectangle')
        return true;
    ...
    if (msg.type === 'apply-multiple-covers-url' && msg.coversUrlData) {
        ...
        for (let i = 0; i < applyCount; i++) {
            ...
            if (logos.length === 0) {
                ...
                console.log(`[provider S2] ancestor="${(_c = instAncestor === null || instAncestor === void 0 ? void 0 : instAncestor.name) !== null && _c !== void 0 ? _c : 'null'}"`);
                if (instAncestor)
                    logos = findProviderLogoNodes([instAncestor], cachedAllCardIds);
            ...
            if (logos.length === 0) {
                ...
                console.log(`[provider S3] providerAncestor="${(_d = providerAncestor === null || providerAncestor === void 0 ? void 0 : providerAncestor.name) !== null && _d !== void 0 ? _d : 'null'}"`);
                if (providerAncestor)
                    logos = [providerAncestor];
            }
            const metaRaw = coverData.metadata;
            const channelName = metaRaw === null || metaRaw === void 0 ? void 0 : metaRaw.channelName;
            const providerFromId = (metaRaw === null || metaRaw === void 0 ? void 0 : metaRaw.contentId) ? extractProvider(metaRaw.contentId) : null;
            const providerValue = channelName || providerFromId;
